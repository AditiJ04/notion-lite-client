...
export default function Share({ documentId }: { documentId: string }) {
  ...
  const loadCollaborators = async () => {
    ...
    const res = await fetch(`${apiUrl}/documents/${documentId}/collaborators`, {
      headers,
    });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const res = await fetch(
        `${apiUrl}/documents/${documentId}/collaborators`,
        { headers },
      );
  ...
  const handleRoleChange = async (collaboratorId: string, newRole: string) => {
    const headers = await getAuthHeader();
    await fetch(
      `${apiUrl}/documents/${documentId}/collaborators/${collaboratorId}`,
      {
        method: "PATCH",
        headers: { ...headers, "Content-Type": "application/json" },
          ...
          >
            ...
            <div className="p-4">
              ...
              <button
                ...
                className="w-full rounded-lg py-2 text-sm font-medium text-white transition-opacity disabled:opacity-40 hover:opacity-90"
                style={{
